...
| Field | Value |
|-------|-------|
| **Current Phase** | _Phase 20 complete._ Ready to start Phase 21. |
| **Next Phase** | [phase-21 — Home Widget Engine](../implementation/phase-21-home-widget-engine.md) (no new infra) |
| **Last completed phase** | 20 — Notification Engine |
| **Contract version** (`@lifeos/contracts`) | `0.8.0` (unchanged; `NotificationEnginePort` is platform-internal) |
| **Database version** (latest migration) | `0014_notifications` |
| **API version** | `v1` (+ `GET /v1/notifications`, `POST /v1/notifications/:id/read`) |
| **Repo state** | `NotificationEngine`: Skills **declare** notification intents against domain events, the engine **decides** delivery. Declarations register as **named idempotent subscribers** (phase-06 dedupe → exactly-once). `NotificationPolicy` (importance floor + quiet hours, injected clock) selects external channels; in-app inbox always persists (CQRS read model), push/email via stub channel adapters (real connectors drop in later, ADR-0005). Unique `dedupeKey` suppresses duplicate notifications. `getInbox` keyset-paginated (RLS user-context), `markRead` user-context. Full gate green (75 tests). Branch `phase-01-foundation`. |
