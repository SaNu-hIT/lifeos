...
| Field | Value |
|-------|-------|
| **Current Phase** | _Phase 18 complete — **M3 (AI brain) done**._ Ready to start Phase 19. |
| **Next Phase** | [phase-19 — Activity Engine](../implementation/phase-19-activity-engine.md) (no new infra) |
| **Last completed phase** | 18 — Workflow Engine |
| **Contract version** (`@lifeos/contracts`) | `0.8.0` (unchanged; `WorkflowEnginePort` is platform-internal) |
| **Database version** (latest migration) | `0012_workflow_runs` |
| **API version** | `v1` (`/v1/health`, `/v1/me`, `/v1/auth/session`, `POST/GET /v1/conversations[...]/messages`) |
| **Repo state** | **M3 complete — full AI brain.** `WorkflowEngine`: durable, resumable multi-step sagas — progress persisted per step, resume-from-step_index on crash, compensation/rollback in reverse on failure; BullMQ worker + boot-time resume of `running` runs. Full gate green (69 tests). Branch `phase-01-foundation`. |
...
Phases 19–36 pending ([roadmap](05_IMPLEMENTATION_ROADMAP.md)). **M1+M2+M3 done (platform spine, plugin framework, AI brain). Next: 19 (Activity), 20 (Notification), 21 (Home Widget), 22 (Skill framework finalization) → then M4 Grocery (23–26).**
