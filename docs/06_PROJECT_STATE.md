...
| Field | Value |
|-------|-------|
| **Current Phase** | _Phase 21 complete._ Ready to start Phase 22. |
| **Next Phase** | [phase-22 — Skill framework finalization](../implementation/phase-22-skill-framework.md) (no new infra) |
| **Last completed phase** | 21 — Home Widget Engine |
| **Contract version** (`@lifeos/contracts`) | `0.8.0` (unchanged; `HomeEnginePort` is platform-internal) |
| **Database version** (latest migration) | `0015_widget_instances` |
| **API version** | `v1` (+ `GET /v1/home`, `PATCH /v1/home/widgets/:key`) |
| **Repo state** | `HomeEngine`: assembles the dynamic home from Skill-contributed widgets at request time — nothing hardcoded. Capability-gated (deny-by-default via `PermissionPort`), user-hidden filtered, each widget `build()`s concurrently (null/throw drops it, never breaks the home), ranked pinned→urgency-boosted-priority / per-user `sortOverride`. `surface.widget_instances` holds per-user overrides (hide/pin/reorder, RLS all-context). `GET /v1/home`, `PATCH /v1/home/widgets/:key`. Full gate green (78 tests). Branch `phase-01-foundation`. **Surface layer (19–21) complete.** |
