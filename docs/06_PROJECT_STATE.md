...
| Field | Value |
|-------|-------|
| **Current Phase** | _Phase 22 complete._ Ready to start Phase 23 (M4 — Grocery Skill). |
| **Next Phase** | [phase-23 — Grocery Skill (part 1)](../implementation/phase-23-grocery-domain.md) (no new infra) |
| **Last completed phase** | 22 — Skill framework finalization |
| **Contract version** (`@lifeos/contracts`) | `0.9.0` (additive: `skill/contributions` — `ActivityProjection`, `NotificationDeclaration`, `WidgetContribution` + data types; manifest gains `activityProjections`) |
| **Database version** (latest migration) | `0015_widget_instances` (no new migration) |
| **API version** | `v1` (unchanged) |
| **Repo state** | `SkillHost`: the **single seam** a Skill plugs into. `install(manifest)` registers (tools + compat + persistence) then wires every runtime contribution into its engine — activity projections → Activity (19), notifications → Notification (20), widgets → Home (21), context providers → Context (13), event handlers → Subscriber Registry (06). The runtime contribution surface is now **canonical in `@lifeos/contracts`** (`skill/contributions.ts`); engine ports re-export it (single source of truth). `INSTALLED_SKILLS` token (composition-root supplied; empty in core, ADR-0001) installed at boot via `OnModuleInit`. Sample Skill enriched to exercise the full surface. Full gate green (81 api + 3 sample tests). Branch `phase-01-foundation`. **Platform complete — ready for the first real Skill.** |
