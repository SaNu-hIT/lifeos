...
| Field | Value |
|-------|-------|
| **Current Phase** | _Phase 19 complete._ Ready to start Phase 20. |
| **Next Phase** | [phase-20 — Notification Engine](../implementation/phase-20-notification-engine.md) (no new infra) |
| **Last completed phase** | 19 — Activity Engine |
| **Contract version** (`@lifeos/contracts`) | `0.8.0` (unchanged; `ActivityEnginePort` is platform-internal) |
| **Database version** (latest migration) | `0013_activities` |
| **API version** | `v1` (+ `GET /v1/activities`) |
| **Repo state** | `ActivityEngine`: append-only feed as a CQRS read model. Projections register as **named idempotent subscribers** (phase-06 dedupe → exactly-once despite at-least-once delivery); `getFeed` keyset-paginated (RLS user-context read; projector inserts service-context). `GET /v1/activities`. Full gate green (71 tests). Branch `phase-01-foundation`. |
