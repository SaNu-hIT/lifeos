---
title: AI Coding Prompt — Phase 13 (Context Engine)
status: Authoritative
phase: 13
type: prompt
---

# Prompt — Phase 13: Context Engine
...
## Role
You are implementing **Phase 13 — Context Engine** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 13 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-13-context-engine.md](../implementation/phase-13-context-engine.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 13 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Assemble the permission-filtered `UnifiedContext` (DB + Memory + Conversation + Settings + Permissions) by composing Skill Context Providers, cached per (user, scope) with invalidation.

## Phase-specific guardrails
Skills consume Context and NEVER query cross-cutting data directly. Never cache across permission boundaries incorrectly (test the filter).

## Universal rules (always)
- Implement **only Phase 13**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 13 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 13 — Context Engine` with the phase **Review Checklist** ticked.

## Dependencies
Phases 07, 12.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-13-context-engine.md](../implementation/phase-13-context-engine.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-14.md](phase-14.md)
