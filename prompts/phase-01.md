---
title: AI Coding Prompt — Phase 01 (Foundation & Monorepo)
status: Authoritative
phase: 1
type: prompt
---

# Prompt — Phase 01: Foundation & Monorepo
...
## Role
You are implementing **Phase 01 — Foundation & Monorepo** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 01 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-01-foundation.md](../implementation/phase-01-foundation.md) — the full spec for THIS phase. This phase's spec is FULLY DETAILED.
...
**Stop and ask a human if:** Phase 01 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Scaffold the Turborepo + pnpm monorepo, shared tsconfig/eslint-config (with the dependency-direction lint rule), Prettier, commitlint, and the CI pipeline. Create placeholder `@lifeos/contracts`, `@lifeos/config`, and an empty `apps/api`.

## Phase-specific guardrails
Add the dependency-direction lint rule WITH a negative test (a forbidden `apps/api`→`skills/*` import must fail lint). Do NOT implement any engine, Skill, or domain logic.

## Universal rules (always)
- Implement **only Phase 01**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 01 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 01 — Foundation & Monorepo` with the phase **Review Checklist** ticked.

## Dependencies
None (first phase).

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-01-foundation.md](../implementation/phase-01-foundation.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-02.md](phase-02.md)
