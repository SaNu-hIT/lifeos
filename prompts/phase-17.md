---
title: AI Coding Prompt — Phase 17 (Planner)
status: Authoritative
phase: 17
type: prompt
---

# Prompt — Phase 17: Planner
...
## Role
You are implementing **Phase 17 — Planner** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 17 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-17-planner.md](../implementation/phase-17-planner.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 17 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Generate validated `ExecutionPlan`s (ordered, dependency-aware tool calls with `requiresUserConfirmation`) from intent + context + available tools, constrained to registered tools and validated against the Tool Registry.

## Phase-specific guardrails
The Planner ONLY produces plans — no execution, no side effects. Reject invalid/unknown tool calls before execution. Tests assert plan STRUCTURE, not LLM prose.

## Universal rules (always)
- Implement **only Phase 17**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 17 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 17 — Planner` with the phase **Review Checklist** ticked.

## Dependencies
Phases 14, 09, 16.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-17-planner.md](../implementation/phase-17-planner.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-18.md](phase-18.md)
