---
title: AI Coding Prompt — Phase 07 (Permission Engine)
status: Authoritative
phase: 7
type: prompt
---

# Prompt — Phase 07: Permission Engine
...
## Role
You are implementing **Phase 07 — Permission Engine** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 07 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-07-permission-engine.md](../implementation/phase-07-permission-engine.md) — the full spec for THIS phase. This phase's spec is FULLY DETAILED.
...
**Stop and ask a human if:** Phase 07 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Implement capability-based authorization: `catalog.capabilities` + `billing.capability_grants`, `PermissionPort.can()`/`capabilitiesFor()`, context filtering hook, Redis capability cache with invalidation, and audit of denials.

## Phase-specific guardrails
Deny-by-default. Grants are source-agnostic (no billing coupling). Honor expiry. Build this BEFORE subscriptions. Bump DB version.

## Universal rules (always)
- Implement **only Phase 07**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 07 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 07 — Permission Engine` with the phase **Review Checklist** ticked.

## Dependencies
Phases 03, 04, 05.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-07-permission-engine.md](../implementation/phase-07-permission-engine.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-08.md](phase-08.md)
