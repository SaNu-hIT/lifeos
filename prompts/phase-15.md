---
title: AI Coding Prompt — Phase 15 (Conversation Engine)
status: Authoritative
phase: 15
type: prompt
---

# Prompt — Phase 15: Conversation Engine
...
## Role
You are implementing **Phase 15 — Conversation Engine** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 15 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-15-conversation-engine.md](../implementation/phase-15-conversation-engine.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 15 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Own conversations/messages/turns/streaming: `conversation.*` repositories, create/append/history (keyset pagination), turn grouping, `turnId` in context, and lifecycle events.

## Phase-specific guardrails
Emit events on message/turn. Keep long threads bounded via summaries (Memory). Bump DB version.

## Universal rules (always)
- Implement **only Phase 15**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 15 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 15 — Conversation Engine` with the phase **Review Checklist** ticked.

## Dependencies
Phases 04, 06.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-15-conversation-engine.md](../implementation/phase-15-conversation-engine.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-16.md](phase-16.md)
