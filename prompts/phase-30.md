---
title: AI Coding Prompt — Phase 30 (Web App (Next.js))
status: Authoritative
phase: 30
type: prompt
---

# Prompt — Phase 30: Web App (Next.js)
...
## Role
You are implementing **Phase 30 — Web App (Next.js)** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 30 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-30-web-app.md](../implementation/phase-30-web-app.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 30 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Build the Next.js user app: auth, streaming conversation + confirmation flow, dynamic home, activity feed, notifications, settings, using an API client generated from OpenAPI and realtime from phase-29.

## Phase-specific guardrails
No business logic in the client — it renders platform state. Home must be dynamic (widgets from enabled Skills).

## Universal rules (always)
- Implement **only Phase 30**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 30 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 30 — Web App (Next.js)` with the phase **Review Checklist** ticked.

## Dependencies
Phases 28, 29.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-30-web-app.md](../implementation/phase-30-web-app.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-31.md](phase-31.md)
