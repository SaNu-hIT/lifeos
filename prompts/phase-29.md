---
title: AI Coding Prompt — Phase 29 (Realtime Layer)
status: Authoritative
phase: 29
type: prompt
---

# Prompt — Phase 29: Realtime Layer
...
## Role
You are implementing **Phase 29 — Realtime Layer** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 29 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-29-realtime.md](../implementation/phase-29-realtime.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 29 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Stream assistant tokens, tool progress, and activity/notification pushes via Supabase Realtime behind `RealtimePort`, with per-user/conversation authorized channels and typed envelopes.

## Phase-specific guardrails
Channels authorized by capability. Every realtime update has a REST catch-up equivalent.

## Universal rules (always)
- Implement **only Phase 29**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 29 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 29 — Realtime Layer` with the phase **Review Checklist** ticked.

## Dependencies
Phase-15.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-29-realtime.md](../implementation/phase-29-realtime.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-30.md](phase-30.md)
