---
title: AI Coding Prompt — Phase 35 (Security Hardening)
status: Authoritative
phase: 35
type: prompt
---

# Prompt — Phase 35: Security Hardening
...
## Role
You are implementing **Phase 35 — Security Hardening** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 35 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-35-security-hardening.md](../implementation/phase-35-security-hardening.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 35 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Run a full security pass: verify the three enforcement layers independently, audit RLS coverage, prove prompt-injection containment, audit secrets/PII handling, add abuse controls, and gate CI on security scanning.

## Phase-specific guardrails
Test each defense layer independently. Prove injected instructions cannot exceed user capabilities. No high-severity scanner findings.

## Universal rules (always)
- Implement **only Phase 35**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 35 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 35 — Security Hardening` with the phase **Review Checklist** ticked.

## Dependencies
Effectively all phases.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-35-security-hardening.md](../implementation/phase-35-security-hardening.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-36.md](phase-36.md)
