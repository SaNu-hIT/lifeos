---
title: AI Coding Prompt — Phase 19 (Activity Engine)
status: Authoritative
phase: 19
type: prompt
---

# Prompt — Phase 19: Activity Engine
...
## Role
You are implementing **Phase 19 — Activity Engine** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 19 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-19-activity-engine.md](../implementation/phase-19-activity-engine.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 19 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Build the append-only activity feed as a CQRS read model fed by idempotent event projections, plus `/v1/activities` (keyset paginated).

## Phase-specific guardrails
Projections idempotent (test duplicate event → single row). `surface.activities` append-only; no sensitive payloads. Bump DB version.

## Universal rules (always)
- Implement **only Phase 19**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 19 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 19 — Activity Engine` with the phase **Review Checklist** ticked.

## Dependencies
Phase-06.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-19-activity-engine.md](../implementation/phase-19-activity-engine.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-20.md](phase-20.md)
