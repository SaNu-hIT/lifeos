---
title: AI Coding Prompt — Phase 32 (Mobile App (Flutter))
status: Authoritative
phase: 32
type: prompt
---

# Prompt — Phase 32: Mobile App (Flutter)
...
## Role
You are implementing **Phase 32 — Mobile App (Flutter)** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 32 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-32-mobile-app.md](../implementation/phase-32-mobile-app.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 32 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Build the Flutter app: auth, streaming conversation, dynamic home, feed, push notifications, settings, against the same platform API (client generated from OpenAPI) with realtime from phase-29.

## Phase-specific guardrails
No business logic in the client. Register push with the Notification Engine. Test on iOS + Android.

## Universal rules (always)
- Implement **only Phase 32**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 32 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 32 — Mobile App (Flutter)` with the phase **Review Checklist** ticked.

## Dependencies
Phases 28, 29 (and 20).

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-32-mobile-app.md](../implementation/phase-32-mobile-app.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-33.md](phase-33.md)
