---
title: AI Coding Prompt — Phase 09 (Tool Registry)
status: Authoritative
phase: 9
type: prompt
---

# Prompt — Phase 09: Tool Registry
...
## Role
You are implementing **Phase 09 — Tool Registry** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 09 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-09-tool-registry.md](../implementation/phase-09-tool-registry.md) — the full spec for THIS phase. This phase's spec is FULLY DETAILED.
...
**Stop and ask a human if:** Phase 09 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Build the registry that registers tools, validates input/output against JSON schemas, enforces capability checks + `requiresConfirmation`, executes handlers, and audits every execution.

## Phase-specific guardrails
A tool with invalid input MUST never reach its handler (test both directions). Every execution is capability-checked and audited. The registry imports no Skill.

## Universal rules (always)
- Implement **only Phase 09**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 09 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 09 — Tool Registry` with the phase **Review Checklist** ticked.

## Dependencies
Phases 03, 07.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-09-tool-registry.md](../implementation/phase-09-tool-registry.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-10.md](phase-10.md)
