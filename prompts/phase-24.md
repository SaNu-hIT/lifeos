---
title: AI Coding Prompt — Phase 24 (Grocery Context & Memory)
status: Authoritative
phase: 24
type: prompt
---

# Prompt — Phase 24: Grocery Context & Memory
...
## Role
You are implementing **Phase 24 — Grocery Context & Memory** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 24 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-24-grocery-context-memory.md](../implementation/phase-24-grocery-context-memory.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 24 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Add Grocery Context Providers (e.g. 'the usual', low-stock) and memory (preferred provider, dietary facts, usual items) so 'order the usual' resolves from memory + context.

## Phase-specific guardrails
Grocery sees only permitted context. Memory is user-editable. No direct cross-cutting DB reads.

## Universal rules (always)
- Implement **only Phase 24**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 24 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 24 — Grocery Context & Memory` with the phase **Review Checklist** ticked.

## Dependencies
Phases 23, 13 (and 12).

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-24-grocery-context-memory.md](../implementation/phase-24-grocery-context-memory.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-25.md](phase-25.md)
