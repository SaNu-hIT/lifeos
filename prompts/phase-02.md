---
title: AI Coding Prompt — Phase 02 (Backend Bootstrap (NestJS))
status: Authoritative
phase: 2
type: prompt
---

# Prompt — Phase 02: Backend Bootstrap (NestJS)
...
## Role
You are implementing **Phase 02 — Backend Bootstrap (NestJS)** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 02 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-02-backend.md](../implementation/phase-02-backend.md) — the full spec for THIS phase. This phase's spec is FULLY DETAILED.
...
**Stop and ask a human if:** Phase 02 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Boot the NestJS app in `apps/api` with versioned `/v1` routing, global ValidationPipe, the `LifeOSError` exception filter + standard envelopes, request-id context, structured redacting logging, typed `@lifeos/config` (fail-fast), and `/v1/health`.

## Phase-specific guardrails
Publish `ApiResponse`/`ApiError`/`LifeOSError` in `@lifeos/contracts` and FREEZE them in PROJECT_STATE. No business logic. Invalid config must prevent boot.

## Universal rules (always)
- Implement **only Phase 02**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 02 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 02 — Backend Bootstrap (NestJS)` with the phase **Review Checklist** ticked.

## Dependencies
Phase-01.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-02-backend.md](../implementation/phase-02-backend.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-03.md](phase-03.md)
