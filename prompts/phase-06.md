---
title: AI Coding Prompt — Phase 06 (Event Bus + Outbox + BullMQ)
status: Authoritative
phase: 6
type: prompt
---

# Prompt — Phase 06: Event Bus + Outbox + BullMQ
...
## Role
You are implementing **Phase 06 — Event Bus + Outbox + BullMQ** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 06 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-06-event-bus.md](../implementation/phase-06-event-bus.md) — the full spec for THIS phase. This phase's spec is FULLY DETAILED.
...
**Stop and ask a human if:** Phase 06 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Build `EventBusPort` (transactional outbox publish), the outbox relay worker → BullMQ, idempotent subscriber infrastructure (dedupe by eventId), retries/backoff, and a dead-letter queue.

## Phase-specific guardrails
Never publish outside the transaction that changes state. Every subscriber MUST be idempotent (test duplicate delivery → single effect).

## Universal rules (always)
- Implement **only Phase 06**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 06 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 06 — Event Bus + Outbox + BullMQ` with the phase **Review Checklist** ticked.

## Dependencies
Phases 03, 04.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-06-event-bus.md](../implementation/phase-06-event-bus.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-07.md](phase-07.md)
