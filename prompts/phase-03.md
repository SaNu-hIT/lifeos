---
title: AI Coding Prompt — Phase 03 (Platform Contracts)
status: Authoritative
phase: 3
type: prompt
---

# Prompt — Phase 03: Platform Contracts
...
## Role
You are implementing **Phase 03 — Platform Contracts** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 03 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-03-platform-contracts.md](../implementation/phase-03-platform-contracts.md) — the full spec for THIS phase. This phase's spec is FULLY DETAILED.
...
**Stop and ask a human if:** Phase 03 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Define the frozen, versioned public interfaces in `@lifeos/contracts`: `Tool`, `UnifiedContext`, `SkillManifest`, `ProviderPort` base, `CapabilityKey`, `DomainEvent`. Ship reusable contract-test suites for each.

## Phase-specific guardrails
Interfaces ONLY — no framework imports in `@lifeos/contracts`. Set the semver compatibility policy and a CI guard for breaking changes. Define only what phases 04–11 need; resist over-engineering.

## Universal rules (always)
- Implement **only Phase 03**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 03 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 03 — Platform Contracts` with the phase **Review Checklist** ticked.

## Dependencies
Phase-02.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-03-platform-contracts.md](../implementation/phase-03-platform-contracts.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-04.md](phase-04.md)
