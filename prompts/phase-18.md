---
title: AI Coding Prompt — Phase 18 (Workflow Engine)
status: Authoritative
phase: 18
type: prompt
---

# Prompt — Phase 18: Workflow Engine
...
## Role
You are implementing **Phase 18 — Workflow Engine** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 18 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-18-workflow-engine.md](../implementation/phase-18-workflow-engine.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 18 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Execute multi-step/long-running/retryable plans durably and resumably on BullMQ: step retries/backoff, idempotency, compensation/rollback, and `await_*` steps.

## Phase-specific guardrails
Non-idempotent steps MUST have compensations. Test crash-resume mid-run.

## Universal rules (always)
- Implement **only Phase 18**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 18 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 18 — Workflow Engine` with the phase **Review Checklist** ticked.

## Dependencies
Phases 06, 17.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-18-workflow-engine.md](../implementation/phase-18-workflow-engine.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-19.md](phase-19.md)
