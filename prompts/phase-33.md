---
title: AI Coding Prompt — Phase 33 (Observability & Audit Logs)
status: Authoritative
phase: 33
type: prompt
---

# Prompt — Phase 33: Observability & Audit Logs
...
## Role
You are implementing **Phase 33 — Observability & Audit Logs** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 33 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-33-observability.md](../implementation/phase-33-observability.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 33 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Add structured logging, distributed tracing (`requestId`/`turnId`), metrics (LLM latency/cost, queue depth, tool error rates), dashboards/alerts, and surface the immutable Audit Log.

## Phase-specific guardrails
No PII/secrets in logs/metrics (redaction list + tests). Sensitive actions audited.

## Universal rules (always)
- Implement **only Phase 33**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 33 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 33 — Observability & Audit Logs` with the phase **Review Checklist** ticked.

## Dependencies
Phase-16.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-33-observability.md](../implementation/phase-33-observability.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-34.md](phase-34.md)
