---
title: AI Coding Prompt — Phase 20 (Notification Engine)
status: Authoritative
phase: 20
type: prompt
---

# Prompt — Phase 20: Notification Engine
...
## Role
You are implementing **Phase 20 — Notification Engine** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 20 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-20-notification-engine.md](../implementation/phase-20-notification-engine.md) — the full spec for THIS phase. This phase's spec is currently a SCAFFOLD — before coding, expand it to the full 17-section structure (use phase-01 as the model) and get it reviewed.
...
**Stop and ask a human if:** Phase 20 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Deliver multi-channel notifications from declared intents: channel adapters behind ports, policy (prefs, quiet hours, batching, importance, dedupe), `surface.notifications`, and `/v1/notifications`.

## Phase-specific guardrails
Skills DECLARE; the engine DELIVERS (no direct channel calls in Skills). Idempotent; respect quiet hours/batching. Bump DB version.

## Universal rules (always)
- Implement **only Phase 20**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 20 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 20 — Notification Engine` with the phase **Review Checklist** ticked.

## Dependencies
Phase-06.

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-20-notification-engine.md](../implementation/phase-20-notification-engine.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-21.md](phase-21.md)
