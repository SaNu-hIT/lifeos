---
title: AI Coding Prompt — Phase 10 (Skill Registry & Skill SDK)
status: Authoritative
phase: 10
type: prompt
---

# Prompt — Phase 10: Skill Registry & Skill SDK
...
## Role
You are implementing **Phase 10 — Skill Registry & Skill SDK** of LifeOS, and nothing else.
...
2. [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md) — confirm Phase 10 is the current/next phase, read frozen contracts + known issues.
3. [implementation/phase-10-skill-registry.md](../implementation/phase-10-skill-registry.md) — the full spec for THIS phase. This phase's spec is FULLY DETAILED.
...
**Stop and ask a human if:** Phase 10 is not the current phase; a dependency from a later phase is missing; the work seems to require changing a **frozen contract**; or a real architectural choice (ADR-worthy) arises.

## Mission
Build the Skill Registry (discover→validate→register→enable/disable, contractVersion compatibility check, forward tools/capabilities), the `@lifeos/skill-sdk` (`defineSkill`, helpers, `runSkillContractTests`), `catalog.skills`, and a trivial sample Skill.

## Phase-specific guardrails
The core MUST NOT import any Skill — manifests only. Refuse incompatible contract versions. Prove the path with the sample Skill's tool executing via the Tool Registry. Bump DB version.

## Universal rules (always)
- Implement **only Phase 10**. Do **not** start, stub, or "prepare" any future phase.
...
4. **Update [docs/06_PROJECT_STATE.md](../docs/06_PROJECT_STATE.md):** move Phase 10 to Completed, set next phase, bump contract/DB/API versions if changed, log known issues, append the change log, set Last Updated.
5. A PR titled `Phase 10 — Skill Registry & Skill SDK` with the phase **Review Checklist** ticked.

## Dependencies
Phases 03, 09 (and 07).

## Definition of Done
Use the agent checklist in [docs/07 §9](../docs/07_AI_AGENT_INSTRUCTIONS.md) **and** this phase's Definition of Done in [implementation/phase-10-skill-registry.md](../implementation/phase-10-skill-registry.md). The app must still build and work.

## After this phase
Next prompt: [prompts/phase-11.md](phase-11.md)
